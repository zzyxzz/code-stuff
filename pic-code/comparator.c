...
{   
    ...
    TRISBbits.TRISB0 = 1;        //portb0 as input
    TRISBbits.TRISB1 = 1;        //portb1 as input
    TRISBbits.TRISB2 = 1;        //portb2 as input
    /*LATBbits.LATB0*/
    
    Rtrans(RMID);//--------------- preset resistance

     
    while(!LATBbits.LATB0)
    {   if(!LATBbits.LATB1 && LATBbits.LATB2)
	    {
        ...
        else if(LATBits.LATB1 && !LATBbits.LATB2)
        {
	        T1CONbits.TON = 1;                   //Timer1 on
		    while(IFS0bits.T1IF)                 //if flag == 1
		    {
