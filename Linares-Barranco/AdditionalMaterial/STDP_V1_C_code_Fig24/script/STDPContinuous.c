 ...
 *
 *  This mex file was originally developped for:
 ...
 *  And adapted to memristor-based STDP in:
 ...
 *  By Tim Masquelier: timothee.masquelier@alum.mit.edu  
 *
...
void LTD(NEURON *neuron, double spikeTime, unsigned short afferent, PARAM *param) {
	...
	(*neuron).weight[afferent] = max( (*param).minWeight, (*neuron).weight[afferent] );
...
void LTP(NEURON *neuron, double firingTime, PARAM *param) {
	...
	for(e=(int)*(*neuron).nEpsp;e>=1;e--) {
		...
		if(!alreadyPotentiated[epspAfferent]) {/* consider only pairs of spikes*/
			...
			(*neuron).weight[epspAfferent] = min(1,(*neuron).weight[epspAfferent]);
...
/* 	for(e=*(*neuron).nEpsp-1; e>*(*neuron).nInefficient-1; e--) { */
	for(e=*(*neuron).nEpsp-1; e>=0; e--) {
        ...
        for(i=0; i< min((*param).nPeriod,(*param).nEpspKernel-shift); i++) 
    ...
    if((*param).inhibStrength>0) {
        for(e=*(*neuron).nIpsp-1; e>=0; e--) {
            ...
            for(i=0; i< min((*param).nPeriod,(*param).nIpspKernel-shift); i++)
    ...
    if((*param).usePssKernel)
        if(*(*neuron).nFiring>0) {
            ...
            for(i=0; i< min((*param).nPeriod,(*param).nPssKernel-shift); i++)
            ...
            if(potential[i] >= (*param).threshold+*(*neuron).trPot) {
                ...
                if(i==0)
                    ...
                    *(*neuron).nextFiring = period[i]-(period[i]-period[i-1])/(potential[i]-potential[i-1])*(potential[i]-(*param).threshold-*(*neuron).trPot);
                    /* *(*neuron).nextFiring = max(time,period[i]-(period[i]-period[i-1])/(potential[i]-potential[i-1])*(potential[i]-(*param).threshold-*(*neuron).trPot)); /* note: max is there to avoid scheduling an event in the past due to rounding pb */                    
                break;
            }
        }
    }
...
void integrate(NEURON *neuron, double time, unsigned short afferent, PARAM *param, bool beSmart) {
	...
	*((*neuron).nEpsp)+=1;
    ...
    if ( *(*neuron).nFiring==0 || time-(*neuron).firingTime[((int)*(*neuron).nFiring-1)%(*neuron).NFIRING] >= (*param).refractoryPeriod )
        (*neuron).epspAmplitude[cursor] = (*neuron).weight[afferent];   
    else
        (*neuron).epspAmplitude[cursor] = 0; /* clamping during the refractory period*/
	...
	if(beSmart)
		if(*(*neuron).maxPotential + (*neuron).weight[afferent] < (*param).threshold+*(*neuron).trPot){ /* threshold not reachable even with most optimistic scenario*/
			*(*neuron).maxPotential += (*neuron).epspAmplitude[cursor]; /* optimistic max */
			return;
		}
				...
				int nrhs, const mxArray *prhs[] )
{
	...
	for(s=0; s<nSpikeList; s++)  { /* main loop */
        ...
        if(s%i_round(max(1,nSpikeList/100.0))==0) {
            mexPrintf(".");
