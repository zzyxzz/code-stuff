...
#####################
dt = 0.05                    ##resolution
time = np.arange(0,2000+dt,dt)   ##simulation time (400) seconds
...
for i,t in enumerate(time):
    ...
    if 200 < t < 880: ##training
        ...
        if len_sp < new_len: ##new post spike arrived
            ...
            count = count + 1
            #print str(count)
            saliv_trace[i] = -np.exp(-(f-6.57)/1.76)*np.exp((-t+saliv_neuron.sptime[new_len-1])/tau)
            if wait_for_post == 1:
        ...
        if index < len(sound_neuron.sptime):
            if t >= sound_neuron.sptime[index]:
                #print str(index)
                index = index +1
                #sound_trace[i] = np.exp((f-14.513)/2.183)*np.exp((-t+sound_neuron.sptime[index -1])/tau)
                if wait_for_pre == 1:
    ...
    if 880 <= t <= 900:  ##probing
        mem_s2.keep_pre(i)
        out_N1 = 0
        out_N2 = sound_trace[i-1] * mem_s2.g[i-1]
        saliv_tr[i] = saliv_neuron.getLifOutOL(out_N1 + out_N2, i, t)
    ...
    if 900 < t < 2000:
        ...
        if len_sp < new_len: ##new post spike arrived
            ...
            countn = countn + 1
            #print str(count)
            saliv_trace[i] = -np.exp(-(ff-6.57)/1.76)*np.exp((-t+saliv_neuron.sptime[new_len-1])/tau)
            if wait_for_post == 1:
                mem_s2.cal_g(sound_trace[i], i)
                print 'g up1' + str(sound_trace[i])
            wait_for_pre = 1
            wait_for_post = 0
        else:
            saliv_trace[i] = -np.exp(-(ff-6.57)/1.76)*np.exp((-t+saliv_neuron.sptime[new_len -1])/tau)
        ...
        if index < len(sound_neuron.sptime):
            if t >= sound_neuron.sptime[index]:
                #print str(index)
                index = index +1
                #sound_trace[i] = np.exp((ff-14.513)/2.183)*np.exp((-t+sound_neuron.sptime[index -1])/tau)
                if wait_for_pre == 1:
                    mem_s2.cal_g(saliv_trace[i], i)
                    print 'g down 1' + str(saliv_trace[i])
                    wait_for_post = 1
                    wait_for_pre = 0
            else:
                #sound_trace[i] = np.exp((ff-14.513)/2.183)*np.exp((-t+sound_neuron.sptime[index -1])/tau)
