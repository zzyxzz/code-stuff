...
Memristor Simulation/trace/net01/dataset/'
...
#init_phi = [0.197, 0.642, 0.467, 0.457, 0.357, 0.243, 0.274, 0.324, 0.096, 0.685]
...
for loop in range(loops):
    ...
    bb = np.array([90.0, 102.5, 173.0, 198.5, 209.0, 223.0, 240.5, 301.0, 317.5, 333.5, 342.5, 353.5, 386.5])
    if loop == loops-1:
        timings.append(np.array([90.0, 102.5, 173.0, 198.5, 209.0, 223.0, 240.5, 301.0, 317.5, 333.5, 342.5, 353.5, 386.5]))
    if len(aa)==len(bb):
    ...
    for ch in range(channelNum):
        ...
        #mem[ch].calPhi(loop+1, -0.05*tmp_area) ##own -----> method  hao
        #mem[ch].calPhi(loop+1, -0.05*tmp_learn_err) ##own -----> method
        mem[ch].calPhi(loop+1, -0.5*span)  ## SPAN ----- method  hao
        ...
        if loop == loops-1:
            ...
            print 'flux' + str(ch) + ' is ' + str(mem[ch].phi[loop]) + '\n'
...
for ch in range(10):
    ...
    plt.plot(xnew,f2(xnew)/max(mse2[ch]))
...
plt.plot(xnew,f1(xnew)/max(mse1))

ax=plt.subplot(313)
plt.title('(c)',fontsize=14)
plt.ylabel(r'Conductance $G$',fontsize=14)
ax.bar([1,4,7,10,13,16,19,22,25,28],pre_weights,color='g',width=1, yerr=c,ecolor='black')
ax.bar([2,5,8,11,14,17,20,23,26,29],after_weights,color='r',width=1, yerr=d,ecolor='black')
