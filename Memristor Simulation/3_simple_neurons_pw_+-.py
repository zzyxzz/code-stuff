...
ref = 4 #16 #20                     # refractory period (msec)
...
def update_synapses(step):
    ...
    for i in range(num_conn):
        ...
        print k,pre,Vo[pre,k],'post =',Vob[post,k]
        s_r = m.r[k-1] + 100
        if (Vo[pre, k] > 0 and Vob[post, k] == 0):
            ...
            m.i[k] = -5.0/s_r #-2.0
            print 'neg double voltage drop'
        elif (Vo[pre, k] == 0 and Vob[post,k] > 0):
            m.i[k] = -2.5/s_r #-1.0
        elif (Vo[pre, k] > 0 and Vob[post,k] < 0):
            m.i[k] = 20.0/s_r #2.0
            print 'double voltage drop'
        elif (Vo[pre, k] == 0 and Vob[post,k] < 0):
            m.i[k] = 2.5/s_r #-1.0
        else:
            m.i[k] = 0.0
        ...
        r = m.r_t(k) 
        print k,pre,m.i[k],m.phi[k],m.q[k],r,'r'           

        synapses[post, pre] = r2w_linear(r) #1.0/r
...
for i, t in enumerate(time[1:],1):
    ...
    print i,active
    Vin[active,i] = Vext[active,i-1] + (synapses[active,:]).dot(Vo[:,i-1])
    print synapses
